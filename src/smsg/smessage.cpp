...
int CSMSG::AddWalletAddresses()
{
    ...
    for (const auto &entry : pwallet->mapAddressBook) { // PAIRTYPE(CTxDestination, CAddressBookData)
        ...
        CKeyID keyID;
        CDynamicAddress coinAddress(entry.first);
...
int CSMSG::ReadIni()
{
    ...
    while (fgets(cLine, 512, fp))  {
        ...
        if (strcmp(pName, "key") == 0) {
            ...
            if (rv == 3) {
                CKeyID k;
                CDynamicAddress(cAddress).GetKeyID(k);
...
int CSMSG::WriteIni()
{
    ...
    for (std::vector<SecMsgAddress>::iterator it = addresses.begin(); it != addresses.end(); ++it) {
        ...
        CDynamicAddress cAddress(it->address);
    ...
    uint32_t &nTransactions, uint32_t &nElements, uint32_t &nPubkeys, uint32_t &nDuplicates)
{
    ...
    for (const auto &tx : block.vtx) {
        ...
        if (!tx->IsDynamicVersion()) // skip legacy txns
            continue;
    ...
    */
    ...
    for (auto &p : smsgModule.keyStore.mapKeys) {
        ...
        if (!(key.nFlags & SMK_RECEIVE_ANON)) {
            ...
            if (Decrypt(false, key.key, address, pHeader, pPayload, nPayload, msg) == 0) {
                if (LogAcceptCategory(BCLog::SMSG)) {
                    LogPrintf("Decrypted message with %s.\n", CDynamicAddress(addressTo).ToString());
                }
    ...
    if (fOwnMessage) {
        ...
        if (!fExisted) {
            ...
            //TODO: Format message
            if (!strCmd.empty()) {
                boost::replace_all(strCmd, "%s", CDynamicAddress(addressTo).ToString());
...
int CSMSG::AddAddress(std::string &address, std::string &publicKey)
{
    ...
    */
    ...
    CDynamicAddress coinAddress(address);
    if (!coinAddress.IsValid()) {
...
int CSMSG::ImportPrivkey(const CDynamicSecret &vchSecret, const std::string &sLabel)
{
    SecMsgKey key;
    ...
    */
    ...
    if (LogAcceptCategory(BCLog::SMSG)) {
        LogPrint(BCLog::SMSG, "SecureMsgEncrypt(%s, %s, ...)\n",
            fSendAnonymous ? "anon" : CDynamicAddress(addressFrom).ToString(),
            CDynamicAddress(addressTo).ToString());
    }
    ...
    if (fSendAnonymous) {
        ...
        vchPayload[0] = (static_cast<CDynamicAddress*>(&coinAddrFrom))->getVersion(); // vchPayload[0] = coinAddrDest.nVersion;
    ...
    */
    ...
    if (LogAcceptCategory(BCLog::SMSG)) {
        LogPrintf("SecureMsgSend(%s, %s, ...)\n",
            fSendAnonymous ? "anon" : CDynamicAddress(addressFrom).ToString(), CDynamicAddress(addressTo).ToString());
    }
    ...
    for (const auto &entry : pwallet->mapAddressBook) { // PAIRTYPE(CTxDestination, CAddressBookData)
        ...
        const CDynamicAddress &address = entry.first;
...
int CSMSG::FundMsg(SecureMessage &smsg, std::string &sError, bool fTestFee, CAmount *nFee)
{
    ...
    txFund.nVersion = DYNAMIC_TXN_VERSION;
...
int CSMSG::Decrypt(bool fTestOnly, const CKey &keyDest, const CKeyID &address, const uint8_t *pHeader, const uint8_t *pPayload, uint32_t nPayload, MessageData &msg)
{
    ...
    */
    ...
    if (LogAcceptCategory(BCLog::SMSG)) {
        LogPrintf("%s: using %s, testonly %d.\n", __func__, CDynamicAddress(address).ToString(), fTestOnly);
    }
    ...
    if (fFromAnonymous) {
        ...
        CDynamicAddress coinAddrFromSig;
